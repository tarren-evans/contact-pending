...
function sound(kind){if(!audioReady)return;switch(kind){
 ...
 case'explosion':noiseBurst(.22,.085,155,0,'lowpass',.65);noiseBurst(.11,.050,520,.012,'bandpass',.8);tone(58,.24,.055,'sine',0,34);tone(92,.09,.026,'triangle',.008,48);break;
...
const PASSIVE_MAX=6,PASSIVE_RADIUS=92,PASSIVE_COSTS=[30,45,68,102,153,230],NAI_LINK_MAX=270,NAI_MIN_AREA=1800,NAI_COLLECT_MS=1000;
function passiveEnabled(){return campaignMission==='ARCADE'||campaignMission==='01-04'||campaignMission==='01-05'}
...
function rebuildNAIs(){
 ...
 if(passiveNAIs.length){log('PASSIVE ISR // '+passiveNAIs.length+' NAI ACTIVE');cpNotice('NAI ESTABLISHED',passiveNAIs.length+' AUTO-COLLECTION ZONE'+(passiveNAIs.length>1?'S':''),'intel')}
}
...
function passiveReset(){passiveCredits=0;passiveDeployMode=false;passiveNextId=1;passiveNodes=[];passiveNAIs=[];['#passiveSensors','#passiveRanges','#passiveNAIs'].forEach(q=>{const g=$(q);if(g)g.textContent=''});const b=$('#passiveDeploy');if(b)b.classList.remove('deploying')}
function passivePlace(p){
 ...
 $('#passiveSensors').append(g);n.el=g;rebuildNAIs();log('PASSIVE ISR // PSR-'+String(n.id).padStart(2,'0')+' DEPLOYED');cpNotice('PASSIVE ISR DEPLOYED','COLLECTION FOOTPRINT ACTIVE','intel');sound('order');ui(true);return true
}
...
pct=100;pulse(col.type);ensureRing(col);log('CLASSIFIED // '+designation(col)+' // '+col.type);sound('classify');col=null;if(campaignMission==='01-02')cpCheck0102();if(campaignMission==='01-03')cpCheck0103();ui(true)},collectMs)}function cpDecisionFloat(t,ok){
...
log('MISSION START // CONTACT IMMINENT 0.6.3.2 OA KESTREL // PASSIVE ISR + NAI')},900)}},850);ui(true)}$('#copyDiag').addEventListener('click',copyDiag);$('#saveDiag').addEventListener('click',saveDiag);$('#reset').addEventListener('click',()=>campaignMission==='ARCADE'?arcadeBegin():deploy());$('#again').addEventListener('click',deploy);document.addEventListener('dblclick',e=>e.preventDefault(),{passive:false});document.addEventListener('wheel',e=>{if(e.ctrlKey)e.preventDefault()},{passive:false});window.visualViewport?.addEventListener('resize',()=>rec('VIEWPORT',`${Math.round(innerWidth)}x${Math.round(innerHeight)} scale=${visualViewport.scale}`));initBlue();running=false;$('#clock').textContent=campaignMission==='ARCADE'?'00:00':'03:00';$('#phase').textContent='MISSION STANDBY';ui(true);$('#startMission').addEventListener('click',()=>{$('#startScreen').style.display='none';deploy()});
